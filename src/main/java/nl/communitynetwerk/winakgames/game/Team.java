...
public class Team {
	...
	public Team(Player leader, String name, Game game) {
		WINAKGames plugin = WINAKGames.getPlugin();
		this.name = name;
	...
	public void invite(Player player) {
		...
		assert leader != null;
		Util.scm(player, WINAKGames.getPlugin().getLang().team_invite_1);
		Util.scm(player, WINAKGames.getPlugin().getLang().team_invite_2.replace("<inviter>", leader.getName()));
		Util.scm(player, WINAKGames.getPlugin().getLang().team_invite_3);
		Util.scm(player, WINAKGames.getPlugin().getLang().team_invite_4);
		pending.add(player.getUniqueId());
		WINAKGames.getPlugin().getPlayerManager().getData(player).setPendingTeam(this);
	}
	...
	public void acceptInvite(Player player) {
		PlayerData playerData = WINAKGames.getPlugin().getPlayerManager().getPlayerData(player);
		playerData.setPendingTeam(null);
		playerData.setTeam(this);
		pending.remove(player.getUniqueId());
		players.add(player.getUniqueId());
		Util.scm(player, WINAKGames.getPlugin().getLang().joined_team);
