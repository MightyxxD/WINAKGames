...
import nl.communitynetwerk.winakgames.events.PlayerJoinGameLIST;
import nl.communitynetwerk.winakgames.events.PlayerLeaveGameLIST;
import nl.communitynetwerk.winakgames.gui.SpectatorGUI;
...
public class GamePlayerData extends Data {
	...
	public void join(Player player) {
		...
		if (status.get() != Status.WAITING && status.get() != Status.STOPPED && status.get() != Status.COUNTDOWN && status.get() != Status.READY) {
			...
			PlayerJoinGameLIST event = new PlayerJoinGameLIST(game, player);
			...
			Bukkit.getScheduler().scheduleSyncDelayedTask(plugin, () -> {
				...
				game.getGameCommandData().runCommands(GameCommandData.CommandType.JOIN, player);
			}, 5);
		}
	}
	...
	public void leave(Player player, Boolean death) {
		Bukkit.getPluginManager().callEvent(new PlayerLeaveGameLIST(game, player, death));
