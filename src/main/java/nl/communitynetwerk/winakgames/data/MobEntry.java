			...
			location.getWorld().spawn(location, entityClass, entity -> {
				...
				if (entity instanceof LivingEntity) {
					...
					if (deathMessage != null)
						mob.setMetadata("death-message", new FixedMetadataValue(WINAKGames.getPlugin(), deathMessage));
				}
			});
		} else {
			MythicMob mob = WINAKGames.getPlugin().getMmMobManager().getMythicMob(mythicMob);
			ActiveMob activeMob = mob.spawn(BukkitAdapter.adapt(location), mythicLevel);
			if (deathMessage != null) {
				activeMob.getEntity().getBukkitEntity().setMetadata("death-message", new FixedMetadataValue(WINAKGames.getPlugin(), deathMessage));
			}
		}
	}
