...
public class ChestDropLIST implements Listener {
	...
	public ChestDropLIST(FallingBlock fb) {
		this.fb = fb;
		this.c = fb.getLocation().getChunk();
		c.load();
		Bukkit.getPluginManager().registerEvents(this, WINAKGames.getPlugin());
	...
	public void onOpenChestDrop(PlayerInteractEvent event) {
		...
		if (event.getAction() == Action.RIGHT_CLICK_BLOCK && beforeBlock != null && event.getClickedBlock().getLocation().equals(beforeBlock.getLocation())) {
			...
			while (c != 0) {
				ItemStack it = WINAKGames.getPlugin().getManager().randomItem(game,false);
				if (it != null) {
					i.addItem(it);
				}
				c--;
