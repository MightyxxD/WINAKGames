...
import nl.communitynetwerk.winakgames.data.KitEntry;
import nl.communitynetwerk.winakgames.utils.NBTAPI;
import nl.communitynetwerk.winakgames.utils.PotionEffectUTIL;
import nl.communitynetwerk.winakgames.utils.PotionTypeUTIL;
...
public class ItemStackMANAGER {
	...
	public ItemStackMANAGER(WINAKGames p) {
		this.plugin = p;
		...
		for (String path : config.getConfigurationSection(gameName + "kits").getKeys(false)) {
			try {
				...
				for (String pot : config.getStringList(gameName + "kits." + path + ".potion-effects")) {
					...
					PotionEffectType type = PotionEffectUTIL.get(poti[0]);
	...
	public ItemStack getItem(String args, boolean isStackable) {
		...
		for (String s : split) {
			...
			if (s.startsWith("enchant:")) {
				...
				PotionEffectUTIL.deprecationWarning(s);
				s = s.replace("potion:", "");
				s = s.replace("potion-type:", "");
				String[] effects = s.split(";");
				for (String effect : effects) {
					PotionEffect potionEffect = PotionEffectUTIL.getPotionEffect(effect);
				...
				PotionData potionData = PotionTypeUTIL.getPotionData(s);
				if (potionData != null) {
