...
public class Placeholders extends PlaceholderExpansion {
	...
	public Placeholders(WINAKGames plugin) {
		this.plugin = plugin;
	...
	public String onRequest(OfflinePlayer player, String identifier) {
		...
		switch (id[0]) {
			...
			case "status":
				return WINAKGames.getPlugin().getManager().getGame(id[1]).getGameArenaData().getStatus().getName();
			case "cost":
				return String.valueOf(WINAKGames.getPlugin().getManager().getGame(id[1]).getGameArenaData().getCost());
			case "playerscurrent":
				return String.valueOf(WINAKGames.getPlugin().getManager().getGame(id[1]).getGamePlayerData().getPlayers().size());
			case "playersmax":
				return String.valueOf(WINAKGames.getPlugin().getManager().getGame(id[1]).getGameArenaData().getMaxPlayers());
			case "playersmin":
				return String.valueOf(WINAKGames.getPlugin().getManager().getGame(id[1]).getGameArenaData().getMinPlayers());
		}
		return null;
	}
