...
import nl.communitynetwerk.winakgames.data.MobEntry;
...
public class MobMANAGER {
	...
	public MobMANAGER(Game game) {
		this.game = game;
		this.config = WINAKGames.getPlugin().getMobConfig().getMobs();
		loadMobs();
	}
		...
		for (String time : Arrays.asList("day", "night")) {
			...
			for (String key : section.getKeys(false)) {
				...
				if (getSection(key) != null) {
					MobEntry entry;
					// MYTHIC MOB
					if (getString(key, "type").startsWith("MM:") && WINAKGames.getPlugin().getMmMobManager() != null) {
					...
					// REGULAR MOB
						...
						for (String pot : getSection(key).getStringList("potion-effects")) {
							...
							PotionEffectType effectType = PotionEffectUTIL.get(poti[0]);
	...
	private ItemStack getItemStack(String key, String section) {
		return WINAKGames.getPlugin().getItemStackManager().getItem(getString(key, section), false);
	}
